...
std::vector<double> statsCal (vector<string> haplotypes, unordered_map<string, int> hapsmap1, unordered_map<string, int> hapsmap2) {
  ...
  for (auto it = hapsmap1.begin(); it != hapsmap1.end(); ++it) {
    hapfreq1 = (double)it -> second / (double)16;
  ...
  for (auto it = hapsmap2.begin(); it != hapsmap2.end(); ++it) {
    hapfreq2 = (double)it -> second / (double)28;
  ...
  for (unsigned i = 0; i < haplotypes.size(); i++) {
    ...
    for (unsigned k = 0; k < line1.length(); k++) {
      if (i < 16) {
	snps1.at(k).at(i) = line1.at(k) - '0';
	snps1values.at(k) += line1.at(k) - '0';
      }
      
      if (i >= 16) {
	snps2.at(k).at(i-16) = line1.at(k) - '0';
  ...
  if (s1 > 1 && s2 > 1) {
    ...
    b1 = (double)(16 + 1) / (double)(3 * 15); 
    b2 = (double)2 * (double)(16 * 16 + 16 + 3) / ((double)9 * (double)16 * (double)(16 - 1)); 
    c1 = b1 - (double)1 / a1;
    c2 = b2 - (double)(16 + 2) / (a1 * (double)16) + a2 / (a1 * a1); 
    e1 = c1 / a1; 
    e2 = c2 / (a1 * a1 + a2);
    pi1 = pairw1 * (double)2 / (double)(16 * 15);
    theta1 = (double)s1 / a1;
    
    tajimasd1 = (pi1 - theta1) / sqrt(abs(e1 * s1 + e2 * s1 * (s1 - (double)1)));
    
    b12 = (double)(28 + 1) / (double)(3 * 27); 
    b22 = (double)2 * (double)(28 * 28 + 28 + 3) / ((double)9 * (double)28 * (double)(28 - 1)); 
    c12 = b12 - (double)1 / a12;
    c22 = b22 - (double)(28 + 2) / (a12 * (double)28) + a22 / (a12 * a12); 
    e12 = c12 / a12; 
    e22 = c22 / (a12 * a12 + a22);
    pi2 = pairw2 * (double)2 / (double)(28 * 27);
    theta2 = (double)s2 / a12;

    tajimasd2 = (pi2 - theta2) / sqrt(abs(e12 * s2 + e22 * s2 * (s2 - (double)1)));

    pairb = pairb / (double)448;
    pairw1 = pairw1 / (double)120;
    pairw2 = pairw2 / (double)378;
    
    if (pairb > 0) {
      fst = (pairb - ((double)4 * pairw1 / (double)11 + (double)7 * pairw2 / (double)11)) / pairb;
    }
    else {
      fst = 0;
    }
	...
	// cout << snps1values.at(i) << "\t" << snps1values.at(j) << "\t" << snps2values.at(i) << "\t" << snps2values.at(j) << endl;
	...
	for (unsigned k = 0; k < snps1.at(0).size() / 2; k++) {
	  // cout << snps1.at(i).at(2*k) << "\t" << snps1.at(i).at(2*k+1) << "\t" << snps1.at(j).at(2*k) << "\t" << snps1.at(j).at(2*k+1) << endl;
...
int main(int argc, char *argv[])
{
  ...
  vector<string> lines; 
  std::vector<std::vector<double> > sumStats(8249, std::vector<double>(36));

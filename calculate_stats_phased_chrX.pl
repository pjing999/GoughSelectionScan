...
#########
# Calculate Pi, Theta, H12, Haplotyp Number, etc
# Needs to set window size, frequency first
#########

# Input file is snps_gough_male9_female5_beagle_map_germany_male8.vcf
# Samples 1 to 9 are Gough males, 10-14 are Gough females, 15 to 22 are German males
  ...
  if (!($_ =~ /^\#/) && !($_ =~ "INDEL")) {
	...
	if ($windowstart == 0 || ($fields[1] - $windowstart > $windowsize)) {
	  if ($windowstart == 0) {
		...
		if ($id == 1) {
		  $p1 = $hapHash{$key} / ($size1);
		}
		elsif ($id == 2) {
		  $p2 = $hapHash{$key} / ($size1);
		}
		
		$hapHeter = $hapHeter - ($hapHash{$key} * $hapHash{$key}) / ($size1 * $size1);
		$h11 += ($hapHash{$key} * $hapHash{$key}) / ($size1 * $size1);
		$id++;
	      }
	      
	      $h121 = $h11 + 2 * $p1 * $p2;
	      $hapfreqhigh1 = max(values %hapHash) / ($size1);
	      $hapfreqmedian1 = median(values %hapHash) / ($size1);
	      
	      # $tmp = length($haplotypes[0]);
	      # print "$snpcount\t$tmp\n";
		...
		if ($id == 1) {
		  $p1 = $hapHash2{$key} / ($size2);
		}
		elsif ($id == 2) {
		  $p2 = $hapHash2{$key} / ($size2);
		}
		
		$hapHeter2 = $hapHeter2 - ($hapHash2{$key} * $hapHash2{$key}) / ($size2 * $size2);
		$h12 += ($hapHash2{$key} * $hapHash2{$key}) / ($size2 * $size2);
		$id++;
	      }
				
	      $h122 = $h12 + 2 * $p1 * $p2;
	      $hapfreqhigh2 = max(values %hapHash2) / ($size2);
	      $hapfreqmedian2 = median(values %hapHash2) / ($size2);
	  ...
	  @haplotypes = ();
	  # if (!($allelecount == $size1 || $allelecount == 0)) {        # Fixed differences don't matter
	  for (my $i = 9; $i < 23; $i++) {
	    @subfields = split(/[\|\:\,]/, $fields[$i]);
	    if ($i >= 18) {
	      $haplotypes[$i - 9 + $i - 18] .= $subfields[0];
	      $haplotypes[$i - 8 + $i - 18] .= $subfields[1];
	    }
	    else {
	      $haplotypes[$i - 9] .= $subfields[0];
	    }
	  }
	  # }
	  ...
	  @haplotypes2 = ();
	  # if (!($allelecount2 == $size2 || $allelecount2 == 0)) {        # Fixed differences don't matter
	  for (my $i = 23; $i < 31; $i++) {
	    @subfields = split(/[\|\:\,]/, $fields[$i]);
	    $haplotypes2[$i - 23] .= $subfields[0];
